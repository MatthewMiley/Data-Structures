...
//Make a function that deletes all nodes with the inputted value in the data
//didnt work because needed to update previous node next value if the current was deleted
template<typename T>
void SLList<T>::searchdelete(const T& val){
    if(empty()){
        return;
    }
    SLLNode<T>* cur = head;
